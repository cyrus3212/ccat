...
import { getAdminUser, addAdminUser } from '../../../api/adminUserApi';
...
class Page extends Component {
  ...
  getUsers = (selectedStoreOption = '') => {
    ...
    this.props.dispatch(getAdminUser(params.code, selectedStoreOption));
  }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    try {
      ...
      if (nextProps.adminUsers.data !== {} && this.state.isAddUser === false || nextProps.adminUsers !== {} && this.state.isSaveUser === true) {
        let result = nextProps.adminUsers.data;
        let users = [];
        ...
        if (this.state.isSaveUser === true && nextProps.adminUsers.isOk === true && this.state.isSendUser === false) {
          this.setState({
        ...
        } else if (this.state.isSaveUser === true && nextProps.adminUsers.isOk === false && this.state.isSendUser === false) {
          this.setState({
            isShowMessageAlert: true,
            messageType: "error",
        ...
        } else if (this.state.isSaveUser === true && this.state.isSendUser === true && nextProps.adminUsers.isOk === true) {
          this.setState({
            isShowMessageAlert: true,
            messageType: "success",
        ...
        } else if (this.state.isSaveUser === true && nextProps.adminUsers.isOk === false && this.state.isSendUser === true) {
          this.setState({
            isShowMessageAlert: true,
            messageType: "error",
  ...
  handleTextInputChange = (event, userId, target) => {
    // console.log('event, userId, target', event, userId, target);
    const { users } = this.state;
    let selectedEmployee = users.findIndex(selectedEmployee => selectedEmployee.id == userId);
  ...
  submitData = () => {
    ...
    this.props.dispatch(addAdminUser(usersData))
  }
  ...
  render() {
    ...
    return (
      ...
      <Fragment>
        ...
        <div className="admin-users-table" style={{marginTop: '40px'}}>
          ...
          <TableEdify data={users} isLoading={this.state.isLoading} tableTitle="User List" rowSelection={false} onRowSelection={this.handleRowSelection} scrollY={650} columns={columns}  />
        </div>
...
Page.propTypes = {
  adminUsers: PropTypes.any.isRequired,
