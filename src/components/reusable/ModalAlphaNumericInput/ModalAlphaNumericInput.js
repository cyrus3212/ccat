...
class ModalAlphaNumericInput extends Component {
  constructor(props) {
    super(props);
    this.state = {
      value: "",
  ...
  render() {
    ...
    return (
      <FormGroup >
        <Row>
          <Col md={12}>
            <div className="alphanumeric-text-input-container">
              <div className="alphanumeric-text-input">
                <TextInput
                  error={error}
                  disabled={disabled}
                  htmlId={htmlId}
                  ...
                  value={value}
                />
                { value && showClear ? <i id="alphanumeric-filtersubmit" className="fa fa-times-circle" onClick={this.handleOnClear} /> : null }
              </div>
            </div>
          </Col>
        </Row>
...
ModalAlphaNumericInput.propTypes = {
...
export default ModalAlphaNumericInput;
